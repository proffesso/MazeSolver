...
1. Клонируйте репозиторий:
   ...
   ```
2. Откройте решение `MazeSolver.sln` в Visual Studio или другой IDE, поддерживающей .NET.
3. Соберите проект (F5 или "Build Solution").
...
1. Запустите приложение из IDE или через командную строку:
   ...
   ```
2. После запуска выберите способ ввода лабиринта:
   - **Файл**: укажите путь к файлу с лабиринтом (например, `maze_test.txt` или `maze_test_simple.txt`). Можно использовать абсолютный или относительный путь, например:
     ...
     ../../../../maze_test.txt
     ```
   - **Вручную**: введите размер лабиринта и строки, представляющие лабиринт (0 — проход, 1 — стена).
...
В проекте присутствуют простые тесты, которые покрывают основные сценарии работы приложения. Расширить тестирование и повысить покрытие кода можно при необходимости.
